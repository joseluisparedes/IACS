...
const badChunk = `function buildSystemPrompt(training: any[]): string {
...
const goodChunk = `function buildSystemPrompt(training: any[]): string {
  const DEFAULT_IDENTITY = \`Eres un Analista de Negocio Senior de TI. Tu tarea es ayudar a los colaboradores a aterrizar y estructurar sus iniciativas o requerimientos de negocio mediante una conversación fluida y profesional. Tu tono es cercano pero formal. Haz preguntas concretas, de una en una, para recopilar toda la información necesaria. No termines la conversación hasta tener respuestas claras para todos los campos requeridos.\`;
...
function getMockSummaryResponse(initialData: any) {
  ...
  return {
    ...
    descripcin_del_problema_o_desafo_situacion_actual: \`La base de contactos actual contiene un alto porcentaje de números inalcanzables (SIP 480/404), afectando negativamente el rendimiento del equipo comercial y generando sobrecostos por intentos fallidos.\`,
...
if (content.includes(badChunk)) {
  ...
  console.log('✅ Cleanly restored buildSystemPrompt, isApiKeyConfigured, and updated mock functions in server.ts');
} else {
  console.error('❌ Could not find exact badChunk');
}
