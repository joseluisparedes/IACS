...
export default function InitiativeDetail() {
  ...
  // Historial consolidado de rondas de observación, subsanación y desestimaciones
  const observationThreads = useMemo(() => {
    ...
    history.forEach((h: any, idx: number) => {
      ...
      if (act.includes('desestimad')) {
        ...
        if (currentThread) {
          ...
          currentThread.answerDetails = h.details?.replace(/^Subsanación:\s*/i, '') || h.details || 'Subsanación completada.';
    ...
    if (currentThread) {
      if (currentObs && currentObs.resolved) {
        currentThread.isResolved = true;
        currentThread.answerDate = currentObs.resolved_at;
        currentThread.answerBy = currentObs.resolved_by;
        currentThread.answerDetails = currentObs.resolution_comment;
        currentThread.answerFiles = currentObs.resolution_files || [];
      }
      threads.push(currentThread);
    } else if (currentObs && !threads.some(t => t.questionDetails === currentObs.details)) {
      threads.push({
        ...
        questionDetails: currentObs.details || '',
        ...
        <div className="lg:col-span-7 xl:col-span-7 space-y-6">
          ...
          <div className="flex items-center gap-1.5 p-1.5 bg-slate-100/80 rounded-2xl border border-slate-200/80 w-full sm:w-fit overflow-x-auto shadow-2xs">
            ...
            >
              ...
              <span>Observaciones y Desestimaciones</span>
              {observationCount > 0 && (
...
{/* ─── Executive Bento Cards (Structured VP Business Case) ─── */}
          ...
          {activeDetailTab === 'observations' && (
            <div className="space-y-6">
              <div className="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
                {/* Encabezado del Tab */}
                <div className="px-6 py-5 bg-gradient-to-r from-amber-50/70 via-orange-50/20 to-white border-b border-amber-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-amber-500 text-white flex items-center justify-center shadow-xs shrink-0">
                    ...
                    <div>
                      <h3 className="text-base font-black text-slate-900 tracking-tight">
                        Historial de Observaciones y Desestimaciones
                      </h3>
                      <p className="text-xs text-slate-500 font-medium mt-0.5">
                        Conversación, acuerdos y respuestas técnicas para trazabilidad institucional.
                      </p>
                    </div>
                  </div>
                ...
                {isObservedState && (
                  <div className="mx-6 mt-5 p-4 rounded-xl bg-amber-50 border border-amber-200 flex items-start gap-3">
                    <AlertTriangle className="w-5 h-5 text-amber-600 shrink-0 mt-0.5 animate-pulse" />
                    <div>
                      <h4 className="text-xs font-bold text-amber-950 uppercase tracking-wider">
                        Estado Actual: Observación Activa en Proceso de Subsanación
                      </h4>
                  ...
                  <div className="mx-6 mt-5 p-4 rounded-xl bg-rose-50 border border-rose-200 flex items-start gap-3">
                    ...
                    <div>
                      <h4 className="text-xs font-bold text-rose-950 uppercase tracking-wider">
                        Estado Actual: Iniciativa Desestimada
                      </h4>
                      <p className="text-xs text-rose-800 mt-0.5 leading-relaxed">
                        El requerimiento fue formalmente desestimado según las justificaciones documentadas a continuación.
                      </p>
                    </div>
                  </div>
                )}
                  ...
                  {observationThreads.length === 0 ? (
                    <div className="text-center py-12 px-4 space-y-3">
                      ...
                      <h4 className="text-sm font-bold text-slate-800">
                        Sin observaciones ni desestimaciones
                      </h4>
                      <p className="text-xs text-slate-500 max-w-md mx-auto leading-relaxed">
                        Esta iniciativa no presenta cuestionamientos pendientes ni antecedentes de desestimación en su ciclo de vida.
                      </p>
                    </div>
                  ) : (
                    observationThreads.map((thread, tIdx) => {
                      ...
                      if (isDesestimacion) {
                        return (
                          <div 
                            key={thread.id || tIdx} 
                            className="rounded-2xl border-2 border-rose-200 bg-rose-50/30 overflow-hidden shadow-xs"
                          >
                            {/* Header Desestimación */}
                            <div className="px-5 py-3 bg-rose-100/60 border-b border-rose-200 flex flex-wrap items-center justify-between gap-2">
                              <div className="flex items-center gap-2">
                                <span className="w-6 h-6 rounded-lg bg-rose-600 text-white text-[11px] font-black flex items-center justify-center">
                                  <Ban className="w-3.5 h-3.5" />
                                </span>
                                <span className="text-xs font-black text-rose-900 uppercase tracking-wider">
                                  Dictamen de Desestimación
                                </span>
                              </div>
                              <div className="flex items-center gap-2">
                                <span className="px-2.5 py-0.5 rounded-md text-[10px] font-bold bg-white text-rose-700 border border-rose-200">
                                  Etapa: {thread.stageName}
                                </span>
                                {thread.questionDate && (
                            ...
                            <div className="p-5 space-y-3">
                              <div className="flex items-center gap-2 text-xs text-slate-600 font-medium">
                                ...
                                <span>Desestimado por: <strong className="text-slate-900">{thread.questionBy}</strong> ({thread.questionRole})</span>
                              </div>
                              <div className="p-4 rounded-xl bg-white border border-rose-200">
                                <span className="text-[10px] font-black uppercase text-rose-800 tracking-wider block mb-1">
                                  Motivo y Justificación de la Desestimación:
                                </span>
                                <p className="text-xs font-semibold text-slate-800 leading-relaxed whitespace-pre-wrap">
                                  {thread.questionDetails}
                                </p>
                              </div>
                            </div>
                          </div>
                      ...
                      return (
                        <div 
                          key={thread.id || tIdx} 
                          className="rounded-2xl border-2 border-slate-200 bg-white overflow-hidden shadow-xs space-y-0"
                        >
                          ...
                          <div className="px-5 py-3.5 bg-slate-50 border-b border-slate-200 flex flex-wrap items-center justify-between gap-2">
                            <div className="flex items-center gap-2.5">
                              <span className="w-6 h-6 rounded-lg bg-amber-500 text-white text-[11px] font-black flex items-center justify-center shadow-2xs">
                                #{thread.roundNumber}
                              </span>
                              <span className="text-xs font-black text-slate-900">
                                Ronda de Observación #{thread.roundNumber}
                              </span>
                            ...
                            <div className="flex items-center gap-2">
                              <span className="px-2.5 py-0.5 rounded-md text-[10px] font-semibold bg-white text-slate-700 border border-slate-200">
                                Origen: {thread.stageName}
                              </span>
                              {thread.isResolved ? (
                                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-black bg-emerald-100 text-emerald-800 border border-emerald-200">
                                  <Check className="w-3 h-3 stroke-[3]" /> Subsanada
                                </span>
                              ) : (
                                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-black bg-amber-100 text-amber-800 border border-amber-200 animate-pulse">
                          ...
                          <div className="p-5 space-y-4">
                            ...
                            <div className="rounded-xl p-4 bg-amber-50/60 border border-amber-200/80 space-y-2">
                              <div className="flex items-center justify-between gap-2 border-b border-amber-200/60 pb-2">
                                <div className="flex items-center gap-2 text-xs font-bold text-amber-950">
                                  <div className="w-6 h-6 rounded-full bg-amber-200 text-amber-800 flex items-center justify-center text-[10px]">
                                    <User className="w-3.5 h-3.5" />
                                  </div>
                                  <span>{thread.questionBy}</span>
                                  <span className="text-[10px] font-medium text-amber-800 px-2 py-0.2 rounded-md bg-amber-100/80 border border-amber-200">
                                    {thread.questionRole}
                                  </span>
                                </div>
                                {thread.questionDate && (
                            ...
                            {thread.isResolved ? (
                              <div className="rounded-xl p-4 bg-emerald-50/50 border border-emerald-200/80 space-y-3 ml-0 sm:ml-4">
                                <div className="flex items-center justify-between gap-2 border-b border-emerald-200/60 pb-2">
                                  <div className="flex items-center gap-2 text-xs font-bold text-emerald-950">
                                    <div className="w-6 h-6 rounded-full bg-emerald-200 text-emerald-800 flex items-center justify-center text-[10px]">
                                      <CheckCircle className="w-3.5 h-3.5" />
                                    </div>
                                    <span>{thread.answerBy || 'Responsable de Subsanación'}</span>
                                    <span className="text-[10px] font-medium text-emerald-800 px-2 py-0.2 rounded-md bg-emerald-100/80 border border-emerald-200">
                                ...
                                <div>
                                  <span className="text-[10px] font-black uppercase text-emerald-900 tracking-wider block mb-1">
                                    Respuesta Técnica de Subsanación:
                                  </span>
                                  <p className="text-xs font-semibold text-slate-800 leading-relaxed whitespace-pre-wrap">
                                    {thread.answerDetails || 'Subsanación técnica registrada exitosamente.'}
                                  </p>
                                </div>
                                  ...
                                  <div className="pt-2 border-t border-emerald-200/60 space-y-2">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                      {thread.answerFiles.map((file: any, fIdx: number) => (
                                        <div 
                                          key={fIdx}
                                          className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white border border-emerald-200 text-xs font-semibold text-slate-800 shadow-2xs hover:border-emerald-400 transition-colors"
                                        >
                                          ...
                                          <a 
                                            ...
                                            target="_blank" 
                                            rel="noopener noreferrer"
                                            className="hover:text-emerald-700 hover:underline max-w-[220px] truncate"
                                            title={file.name}
                                          >
                                            {file.name}
                                          </a>
                              ...
                              <div className="rounded-xl p-4 bg-slate-50 border border-dashed border-amber-300 flex items-center gap-3 ml-0 sm:ml-4">
                                <Clock className="w-5 h-5 text-amber-600 shrink-0" />
                                <div>
                                  <h5 className="text-xs font-bold text-slate-800">
                                    Subsanación en Proceso
                                  </h5>
                                  <p className="text-xs text-slate-500 mt-0.5">
                                    Esperando respuesta técnica y documentos de soporte por parte de los roles asignados a la caja de Observación ({observadaResponsibleRoleName}).
                                  </p>
                                </div>
                              </div>
                            )}
